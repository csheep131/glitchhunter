...
- Gate 0: Evidence Contract Validation (NEW)
...
from agent.evidence_gate import EvidenceGate, GateValidationResult, GateDecision
from agent.hypothesis_agent import HypothesisAgent, BugCandidate
...
class PatchLoopState:
    ...
    Attributes:
        ...
        # Evidence Contract fields (NEW)
        evidence_package: Evidence package generated by HypothesisAgent.
        evidence_result: Result from EvidenceGate validation.
        evidence_passed: Whether evidence validation passed.
    """
    ...
    evidence_result: Optional[GateValidationResult] = None
...
class PatchLoopStateMachine:
    ...
    Ablauf mit Evidence-Contract:
    0. Generiere Evidence-Paket (HypothesisAgent)
    1. EvidenceGate Validation (Gate 0) - MUST PASS before proceeding
    2. Generiere Regression-Tests (Fail2Pass)
    3. Generiere Patch
    4. Gate 1: Pre-Apply Validation
    5. Sandbox Execution (Tests ausführen)
    6. Gate 2: Post-Apply Verification
    7. Entscheidung: ACCEPT / RETRY / ESCALATE

    Usage:
        machine = PatchLoopStateMachine(candidate, original_code, data_flow_graph, cfg)
        result = machine.run()
    """
    ...
    def __init__(
        ...
        evidence_gate_config: Optional[Dict[str, Any]] = None,
    ) -> None:
        """
        ...
        Args:
            ...
            data_flow_graph: Data-Flow-Graph für Evidence-Generierung.
            cfg: Control-Flow-Graph für Evidence-Generierung.
            evidence_gate_config: Konfiguration für EvidenceGate.
        """
        ...
        # Evidence-Contract Komponenten (NEW)
        self.hypothesis_agent = HypothesisAgent()
        self.evidence_gate = EvidenceGate(config=evidence_gate_config)
    ...
    def _build_graph(self) -> StateGraph:
        """
        Baut LangGraph State Machine mit Evidence-Contract.

        Returns:
            Configured StateGraph
        """

        def generate_evidence_package(state: Dict[str, Any]) -> Dict[str, Any]:
            """Generiere Evidence-Paket für Bug-Kandidat."""
            logger.info("Generiere Evidence-Paket (Gate 0)")
            
            # Candidate in BugCandidate konvertieren
            candidate_obj = self._dict_to_bug_candidate(state["candidate"])
            
            # Evidence-Paket generieren
            evidence_package = self.hypothesis_agent.generate_evidence_package(
            ...
            state["evidence_passed"] = False  # Wird nach Validation gesetzt
            
            logger.info(
                f"Evidence-Paket generiert: score={evidence_package.evidence_score:.2f}, "
                f"strength={evidence_package.evidence_strength.value}"
            )
            
            return state

        def validate_evidence_gate(state: Dict[str, Any]) -> Dict[str, Any]:
            """Validiere Evidence-Paket durch EvidenceGate."""
            logger.info("EvidenceGate Validation (Gate 0)")
            
            evidence_package = state.get("evidence_package")
            if not evidence_package:
                state["evidence_passed"] = False
                state["evidence_error"] = "Kein Evidence-Paket verfügbar"
                state["evidence_result"] = GateValidationResult(
                    passed=False,
                    decision=GateDecision.REJECTED,
                    errors=["Evidence package missing"],
                )
                return state
            
            # EvidenceGate validation
            result = self.evidence_gate.validate(evidence_package)
            state["evidence_result"] = result
            state["evidence_passed"] = result.passed
            
            if not result.passed:
                logger.warning(
                    f"EvidenceGate validation failed: {result.errors}. "
                    f"Decision: {result.decision.value}"
                )
            else:
                logger.info("EvidenceGate validation passed")
            
            return state

        def generate_regression_tests(state: Dict[str, Any]) -> Dict[str, Any]:
            """Generiere Regression-Tests (Fail2Pass)."""
        ...
        # Workflow definieren mit Evidence-Contract
        workflow = StateGraph(dict)

        # Evidence-Contract Nodes (NEW - Gate 0)
        workflow.add_node("generate_evidence_package", generate_evidence_package)
        workflow.add_node("validate_evidence_gate", validate_evidence_gate)
        ...
        # Entry Point: Evidence Package Generation
        workflow.set_entry_point("generate_evidence_package")

        # Evidence-Contract Flow
        workflow.add_edge("generate_evidence_package", "validate_evidence_gate")
        
        # Conditional Edge nach EvidenceGate
        workflow.add_conditional_edges(
            "validate_evidence_gate",
            lambda state: "passed" if state.get("evidence_passed", False) else "retry_evidence",
            {
                "passed": "generate_regression_tests",  # Proceed to Patch Loop
                "retry_evidence": "generate_evidence_package",  # Retry evidence generation
            },
        )

        # Existing Edges (Patch Loop)
    ...
    def _dict_to_bug_candidate(self, candidate_dict: Dict[str, Any]) -> BugCandidate:
        """Konvertiert Dict zu BugCandidate für Evidence-Generierung."""
        from agent.hypothesis_agent import Severity
        
        return BugCandidate(
            id=candidate_dict.get("id", candidate_dict.get("candidate_id", "unknown")),
            file_path=candidate_dict.get("file_path", ""),
            line=candidate_dict.get("line", candidate_dict.get("line_start", 0)),
